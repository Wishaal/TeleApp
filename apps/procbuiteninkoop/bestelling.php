...
switch ($action) {
    ...
    case 'update';
        ...
        if ($input->input('bstl_co_logistiek') == 'Accoord') {
            ...
            sendEmail($mailadress, null, 'H-IM: Nieuw goedgekeurde aanvraag!', $html, null); //send mail function

            setAanvraagLog($_GET['recordId'], "H-IM: Nieuw goedgekeurde aanvraag!", getProfileNameSession());
            ...
            require_once 'templates/email/aanvraag_voorbereider_niet_accoord.template.php'; //accoord template
            sendEmail($mailadress, null, 'H-IM: Nieuw afgekeurde aanvraag!', $html, null); //send mail function

            setAanvraagLog($_GET['recordId'], "H-IM: Nieuw afgekeurde aanvraag!<br>Opmerking:" . $opmerking, getProfileNameSession());
        }
        if ($input->input('bstl_hoofd_inventory') == 'Accoord') {
            ...
            sendEmail($mailadress, null, 'H-PROV: Nieuw goedgekeurde aanvraag!', $html, null); //send mail function

            setAanvraagLog($_GET['recordId'], "H-PROV: Nieuw goedgekeurde aanvraag!", getProfileNameSession());
            ...
            if ($input->input('terug_naar') == 'Voorbereider') {
                ...
                $add = EmailGroup::whereName('NA_INVENTORY')->first(); // get group name id
                $mailadress = EmailUser::whereEmailGroupId($add->id)->get(); //get users array
                $aanvraagnremail = $bestelling->aanvraag_nr; //set aanvraag nummer
                $artikelomschrijving = $bestelling->artikelInfo->artikelomschrijving; //set artikel omschrijving
                require_once 'templates/email/aanvraag_voorbereider_niet_accoord.template.php'; //accoord template
                sendEmail($mailadress, null, 'H-PROV: Nieuwe niet geaccordeerde aanvraag!', $html, null); //send mail function
                setAanvraagLog($_GET['recordId'], "H-PROV: Nieuwe niet geaccordeerde aanvraag naar INVENTORY!<br>Opmerking:" . $opmerking, getProfileNameSession());
                ...
                $opmerking = $input->input('bstl_hoofd_inventory_opmerking');
                $add = EmailGroup::whereName('NA_HIM')->first(); // get group name id
                $mailadress = EmailUser::whereEmailGroupId($add->id)->get(); //get users array
                $aanvraagnremail = $bestelling->aanvraag_nr; //set aanvraag nummer
                $artikelomschrijving = $bestelling->artikelInfo->artikelomschrijving; //set artikel omschrijving
                require_once 'templates/email/aanvraag_voorbereider_niet_accoord.template.php'; //accoord template
                sendEmail($mailadress, null, 'H-PROV: Nieuwe niet geaccordeerde aanvraag naar HIM!', $html, null); //send mail function
                setAanvraagLog($_GET['recordId'], "H-PROV: Nieuwe niet geaccordeerde aanvraag!<br>Opmerking:" . $opmerking, getProfileNameSession());
            }
            ...
            $artikelomschrijving = $bestelling->artikelInfo->artikelomschrijving; //set artikel omschrijving
            require_once 'templates/email/aanvraag_voorbereider_niet_accoord.template.php'; //accoord template
            sendEmail($mailadress, null, 'H-PROV: Afgekeurde aanvraag!', $html, null); //send mail function

            setAanvraagLog($_GET['recordId'], "H-PROV: Afgekeurde aanvraag!<br>Opmerking:" . $opmerking, getProfileNameSession());
        }
...
if (getInkoopPermisson('Voorbereider')) {
    $bestelling = Bestelling::where('bstl_voorbereider', '!=', 'Accoord')->orWhereNull('bstl_voorbereider')->get();
    ...
    $bestelling = Bestelling::whereHas('aanvraagStatus', function ($query) {
        ...
        $query->Where('bstl_hoofd_inventory', '=', null);
        //$query->orWhere('bstl_hoofd_inventory', '=', 'Geen Accoord');
        $query->orWhere('bstl_hoofd_inventory', '=', '');
    })->get();
}

require_once('tmpl/bestelling.tpl.php');
