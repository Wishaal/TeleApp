    ...
    <section class="content">
        ...
        <div class="box animated bounceInRight">
            <div class="box-header with-border">
                <h3 class="box-title">Statussen </h3>
            </div>
            <div class="box-body">
                ...
                <?php } ?>
                <a data-toggle="modal" href="apps/<?php echo app_name; ?>/modals/status.php?action=new"
                ...
                <table id="mainTable" class="table table-bordered table-striped dataTable">
                    ...
                    $count = 1;
                    foreach ($status as $r) {
                        echo '<tr class="clickable-row" id =' . $r->id . ' naam ="' . $r->status_omschrijving . '"">
                    <td>' . $count . '</td>
                    <td>' . $r->status_omschrijving . '</td>
                    <td>' . $r->kenmerk . '</td>
                    '; ?>
                        </td>
                        </tr>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#mainTable tr').click(function (event) {
                ...
                $('.inputDisabled').attr('href', "apps/<?php echo app_name;?>/modals/status.php?action=update&id=" + $(this).attr('id'));

                $('.inputDisabledDelete').prop("disabled", false); // Element(s) are now enabled.
                $('.inputDisabledDelete').attr('data-href', "apps/<?php echo app_name;?>/status.php?action=delete&id=" + $(this).attr('id'));
