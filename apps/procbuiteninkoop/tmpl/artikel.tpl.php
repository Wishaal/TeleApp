    ...
    <section class="content">
        ...
        <div class="box animated bounceInRight">
            <div class="box-header with-border">
                <h3 class="box-title">Artikelen </h3>
            </div>
            <div class="box-body">
                ...
                <?php } ?>
                <a data-toggle="modal" href="apps/<?php echo app_name; ?>/modals/artikel_import.php?action=import"
                ...
                <table id="mainTable" class="table table-bordered table-striped dataTable">
                    ...
                    <?php
                    $count = 1;
                    foreach ($artikels as $r) {
                        echo '<tr class="clickable-row" id =' . $r->id . ' naam ="' . $r->artikelomschrijving . '"">
                    <td>' . $count . '</td>
                    <td>' . $r->artikel . '</td>
                    <td>' . $r->artikelomschrijving . '</td>
                    <td>' . $r->eenheid . '</td>
                    <td>' . $r->geleverd . '</td>
                    <td>' . $r->jaar . '</td>
                    '; ?>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#mainTable tr').click(function (event) {
                ...
                $('.inputDisabled').attr('href', "apps/<?php echo app_name;?>/modals/artikel.php?action=update&id=" + $(this).attr('id'));

                $('.inputDisabledDelete').prop("disabled", false); // Element(s) are now enabled.
                $('.inputDisabledDelete').attr('data-href', "apps/<?php echo app_name;?>/artikel.php?action=delete&id=" + $(this).attr('id'));
