    ...
    <section class="content">
        ...
        <div class="box animated bounceInRight">
            ...
            <div class="box-body">
                ...
                <?php } ?>
                <a data-toggle="modal" href="apps/<?php echo app_name; ?>/modals/user.php?action=new"
                ...
                <table id="mainTable" class="table table-bordered table-striped dataTable">
                    ...
                    $count = 1;
                    foreach ($users as $r) {
                        echo '<tr class="clickable-row" id =' . $r->id . ' naam ="' . $r->username . '"">
                    <td>' . $count . '</td>
                    <td>' . $r->username . '</td>
                    <td>' . $r->created_at . '</td>
                    <td>' . $r->updated_at . '</td>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#mainTable tr').click(function (event) {
                ...
                $('.inputDisabled').attr('href', "apps/<?php echo app_name;?>/modals/user.php?action=update&id=" + $(this).attr('id'));

                $('.inputDisabledDelete').prop("disabled", false); // Element(s) are now enabled.
                $('.inputDisabledDelete').attr('data-href', "apps/<?php echo app_name;?>/users.php?action=delete&id=" + $(this).attr('id'));
