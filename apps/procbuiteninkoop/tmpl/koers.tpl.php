    ...
    <section class="content">
        ...
        <div class="box animated bounceInRight">
            ...
            <div class="box-body">
                ...
                <?php } ?>
                <a data-toggle="modal" href="apps/<?php echo app_name; ?>/modals/koers.php?action=new"
                ...
                <table id="mainTable" class="table table-bordered table-striped dataTable">
                    ...
                    $count = 1;
                    foreach ($koers as $r) {
                        echo '<tr class="clickable-row" id =' . $r->id . ' naam ="' . $r->koers . '"">
                    <td>' . $count . '</td>
                    <td>' . $r->usd_koers . '</td>
                    <td>' . $r->euro_koers . '</td>
                    <td>' . $r->omrekeningskoers . '</td>
                    <td>' . $r->datum . '</td>
                    '; ?>
                        </td>
                        </tr>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#mainTable tr').click(function (event) {
                ...
                $('.inputDisabled').attr('href', "apps/<?php echo app_name;?>/modals/koers.php?action=update&id=" + $(this).attr('id'));

                $('.inputDisabledDelete').prop("disabled", false); // Element(s) are now enabled.
                $('.inputDisabledDelete').attr('data-href', "apps/<?php echo app_name;?>/koers.php?action=delete&id=" + $(this).attr('id'));
