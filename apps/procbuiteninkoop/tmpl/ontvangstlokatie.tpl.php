    ...
    <section class="content">
        ...
        <div class="box animated bounceInRight">
            <div class="box-header with-border">
                <h3 class="box-title">Ontvangst Lokatie </h3>
            </div>
            <div class="box-body">
                ...
                <?php } ?>
                <a data-toggle="modal" href="apps/<?php echo app_name; ?>/modals/ontvangstlokatie.php?action=new"
                ...
                <table id="mainTable" class="table table-bordered table-striped dataTable">
                    ...
                    $count = 1;
                    foreach ($koers as $r) {
                        echo '<tr class="clickable-row" id =' . $r->id . ' naam ="' . $r->lokatie . '"">
                    <td>' . $count . '</td>
                    <td>' . $r->lokatie . '</td>
                    '; ?>
                        </td>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#mainTable tr').click(function (event) {
                ...
                $('.inputDisabled').attr('href', "apps/<?php echo app_name;?>/modals/ontvangstlokatie.php?action=update&id=" + $(this).attr('id'));

                $('.inputDisabledDelete').prop("disabled", false); // Element(s) are now enabled.
                $('.inputDisabledDelete').attr('data-href', "apps/<?php echo app_name;?>/ontvangstlokatie.php?action=delete&id=" + $(this).attr('id'));
