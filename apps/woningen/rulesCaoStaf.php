...
$aantalGoedGekeurdeRegistratie = aantalGoedekeurdeRegistratie($db,$begin,$_PO45ST['badgenr']);
$checkIfSameDateRegistration = checkIfSameDateRegistration($db,$begin,$eind,$_POST['badgenr']);
...
if($aantalGoedGekeurdeRegistratie['aantal'] <= 5){
    if(empty($checkIfSameDateRegistration)){
        if(empty($checkIfRegistrationExist) || $wachtlijst == 1){
             $final = finalInsert($db,$_POST['w_id'],$_POST['daterange'],$_POST['badgenr']);
