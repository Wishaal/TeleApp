        ...
        <div class="box">
            ...
            <div class="box-body">
                ...
                <?php if (hasPermissionPDO($mis_connPDO, $activemenuitem['id'], 'INSERT')) { ?>
                    <a data-toggle="modal" href="apps/<?php echo app_name; ?>/modals/aanvragenparts.php?action=new"
                ...
                <table id="mainTable" class="table table-bordered table-striped dataTable">
                    ...
                    $count = 1;
                    foreach ($Aanvragenparts as $item) {
                        echo '<tr class="clickable-row" id =' . $item->aanvpartnr . ' naam ="' . $item->aanvpartnr . '">
                            <td>' . $count . '</td>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#mainTable tr').click(function (event) {
                ...
                $('.inputDisabled').attr('href', "apps/<?php echo app_name;?>/modals/aanvragenparts.php?action=update&id=" + $(this).attr('id'));

                $('.inputDisabledDelete').prop("disabled", false); // Element(s) are now enabled.
                $('.inputDisabledDelete').attr('data-href', "apps/<?php echo app_name;?>/aanvragenparts.php?action=delete&id=" + $(this).attr('id'));
