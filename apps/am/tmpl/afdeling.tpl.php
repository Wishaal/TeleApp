        ...
        <div class="box">
            ...
            <div class="box-body">
                ...
                <?php if (hasPermissionPDO($mis_connPDO, $activemenuitem['id'], 'INSERT')) { ?>
                    <a data-toggle="modal" href="apps/<?php echo app_name; ?>/modals/afdeling.php?action=new"
                ...
                <table id="mainTable" class="table table-bordered table-striped dataTable">
                    ...
                    $count = 1;
                    foreach ($Afdelingen as $item) {
                        echo '<tr class="clickable-row" id =' . $item->afdelingcode . ' naam ="' . $item->afdelingcode . '">
                            <td>' . $count . '</td>
                            <td>' . $item->afdelingcode . '</td>
                            <td>' . $item->afdelingnaam . '</td>
                            <td>' . $item->onderdirectoraat . '</td>
                            <td>' . $item->kostenplaatscode . '</td>
                          </tr>';
                        $count++;
                    }
                    ?>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#mainTable tr').click(function (event) {
                ...
                $('.inputDisabled').attr('href', "apps/<?php echo app_name;?>/modals/afdeling.php?action=update&id=" + $(this).attr('id'));

                $('.inputDisabledDelete').prop("disabled", false); // Element(s) are now enabled.
                $('.inputDisabledDelete').attr('data-href', "apps/<?php echo app_name;?>/afdeling.php?action=delete&id=" + $(this).attr('id'));
