        ...
        <div class="box">
            ...
            <div class="box-body">
                ...
                <?php if (hasPermissionPDO($mis_connPDO, $activemenuitem['id'], 'INSERT')) { ?>
                    <a data-toggle="modal" href="apps/<?php echo app_name; ?>/modals/district.php?action=new"
                ...
                <table id="mainTable" class="table table-bordered table-striped dataTable">
                    ...
                    $count = 1;
                    foreach ($Districten as $item) {
                        echo '<tr class="clickable-row" id =' . $item->district_id . ' naam ="' . $item->district_id . '">
                            <td>' . $count . '</td>
                            <td>' . $item->district_code . '</td>
                            <td>' . $item->district_naam . '</td>
                          </tr>';
                        $count++;
                    }
                    ?>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#mainTable tr').click(function (event) {
                ...
                $('.inputDisabled').attr('href', "apps/<?php echo app_name;?>/modals/district.php?action=update&id=" + $(this).attr('id'));

                $('.inputDisabledDelete').prop("disabled", false); // Element(s) are now enabled.
                $('.inputDisabledDelete').attr('data-href', "apps/<?php echo app_name;?>/district.php?action=delete&id=" + $(this).attr('id'));
