        ...
        <div class="box">
            ...
            <div class="box-body">
                ...
                <?php if (hasPermissionPDO($mis_connPDO, $activemenuitem['id'], 'INSERT')) { ?>
                    <a data-toggle="modal" href="apps/<?php echo app_name; ?>/modals/werkorder.php?action=new"
                ...
                <table id="mainTable" class="table table-bordered table-striped dataTable">
                    ...
                    $count = 1;
                    foreach ($Werkorders as $item) {
                        echo '<tr class="clickable-row" id =' . $item->aanvraagnr . ' naam ="' . $item->aanvraagnr . '">
                            <td>' . $count . '</td>
                            <td>' . $item->aanvraagnr . '</td>
							<td>' . $item->volgnr . '</td>
							<td>' . $item->afdelingcode . '</td>
							<td>' . $item->taaknaam . '</td>
							<td>' . $item->taakverstuurd . '</td>
							<td>' . $item->taakafgemeld . '</td>
							<td>' . $item->badgenr . '</td>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#mainTable tr').click(function (event) {
                ...
                $('.inputDisabled').attr('href', "apps/<?php echo app_name;?>/modals/werkorder.php?action=update&id=" + $(this).attr('id'));

                $('.inputDisabledDelete').prop("disabled", false); // Element(s) are now enabled.
                $('.inputDisabledDelete').attr('data-href', "apps/<?php echo app_name;?>/werkorder.php?action=delete&id=" + $(this).attr('id'));
